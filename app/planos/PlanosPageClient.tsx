...
export default function PlanosPageClient() {
  ...
  return (
    <div className="overflow-hidden">
      <section className="border-b border-[var(--border-color)] bg-[var(--bg-base)]">
        <div className="container grid min-h-[calc(100dvh-4rem)] items-center gap-12 py-16 lg:grid-cols-[1fr_0.9fr] lg:py-24">
          <div className="max-w-3xl">
            <span className="inline-flex items-center gap-2 rounded-full border border-[var(--border-color)] bg-[var(--bg-surface)] px-3 py-1 text-xs font-semibold uppercase tracking-wider text-[var(--primary)]">
              <SparkIcon />
              Foco no ENEM Max
            </span>
            <h1 className="mt-7 text-4xl font-semibold tracking-tight text-[var(--text-primary)] sm:text-5xl lg:text-6xl">
            ...
            <div className="mt-9 flex flex-wrap items-center gap-3">
              {status.authenticated ? (
                <button
                  ...
                  className="inline-flex items-center justify-center rounded-xl bg-[var(--primary)] px-6 py-3 text-sm font-semibold text-white shadow-md transition-colors hover:bg-[var(--primary-hover)] disabled:cursor-not-allowed disabled:opacity-60"
                >
                  {action === 'checkout'
                    ? 'Abrindo checkout...'
                    : action === 'portal'
                ...
                <Link
                  href="/register"
                  className="inline-flex items-center justify-center rounded-xl bg-[var(--primary)] px-6 py-3 text-sm font-semibold text-white shadow-md transition-colors hover:bg-[var(--primary-hover)]"
                >
              ...
              <Link
                ...
                className="inline-flex items-center justify-center rounded-xl border border-[var(--border-color)] px-6 py-3 text-sm font-semibold text-[var(--text-secondary)] transition-colors hover:bg-[var(--bg-elevated)] hover:text-[var(--text-primary)]"
              >
            ...
            {(statusMessage || error) && (
              <p
                className={`mt-5 rounded-xl border px-4 py-3 text-sm ${
                  error
                    ? 'border-[var(--danger)]/25 bg-[var(--danger-light)] text-[var(--danger)]'
                    : 'border-[var(--success)]/25 bg-[var(--success-light)] text-[var(--success)]'
                }`}
              >
                {error || statusMessage}
              </p>
          ...
          <div className="relative">
            <div className="rounded-[2rem] border border-[var(--border-color)] bg-[var(--card-bg)] p-5 shadow-2xl">
              ...
              <div className="grid gap-4 pt-5 sm:grid-cols-2">
                {MAX_PLAN_BENEFITS.map((benefit) => (
                  <div key={benefit.title} className="rounded-2xl border border-[var(--border-color)] bg-[var(--bg-base)] p-4">
                    <div className="mb-3 inline-flex h-9 w-9 items-center justify-center rounded-xl bg-[var(--primary)] text-white">
                      <SparkIcon />
                    </div>
                    <h3 className="text-sm font-semibold text-[var(--text-primary)]">{benefit.title}</h3>
                    <p className="mt-2 text-xs leading-relaxed text-[var(--text-muted)]">{benefit.description}</p>
                  </div>
                ))}
              </div>
              <div className="mt-5 rounded-2xl border border-[var(--border-color)] bg-[var(--bg-surface)] p-4">
                <div className="flex items-center justify-between gap-4">
                  ...
                  <div className="text-right">
                    <p className="text-xs font-semibold uppercase tracking-wider text-[var(--text-muted)]">Renovação</p>
                    <p className="mt-1 text-sm font-semibold text-[var(--text-primary)]">{formatDate(subscription.renewsAt)}</p>
                  </div>
                </div>
              </div>
            </div>
      ...
      <section className="bg-[var(--bg-surface)] py-16 md:py-24">
        <div className="container">
          ...
          <div className="grid gap-5 lg:grid-cols-2">
            <article className="rounded-[1.5rem] border border-[var(--border-color)] bg-[var(--card-bg)] p-6 md:p-8">
              <p className="text-sm font-semibold text-[var(--text-muted)]">{FREE_PLAN_NAME}</p>
              <div className="mt-4 flex items-end gap-2">
                <span className="text-4xl font-semibold tracking-tight text-[var(--text-primary)]">{FREE_PLAN_PRICE_DISPLAY}</span>
                <span className="pb-1 text-sm text-[var(--text-muted)]">para usar</span>
              </div>
              ...
              <Link
                href="/register"
                className="mt-8 inline-flex w-full items-center justify-center rounded-xl border border-[var(--border-color)] px-5 py-3 text-sm font-semibold text-[var(--text-secondary)] transition-colors hover:bg-[var(--bg-elevated)] hover:text-[var(--text-primary)]"
              >
                Começar gratuito
              </Link>
            </article>

            <article className="rounded-[1.5rem] border border-[var(--primary)]/40 bg-[var(--card-bg)] p-6 shadow-xl md:p-8">
              <div className="flex items-center justify-between gap-4">
                <p className="text-sm font-semibold text-[var(--primary)]">{MAX_PLAN_MARKETING.name}</p>
                <span className="rounded-full bg-[var(--primary)] px-3 py-1 text-xs font-semibold text-white">
                  Mais escolhido
                </span>
              </div>
              <div className="mt-4 flex items-end gap-2">
                <span className="text-4xl font-semibold tracking-tight text-[var(--text-primary)]">{MAX_PLAN_MARKETING.price}</span>
              </div>
              <p className="mt-4 text-sm text-[var(--text-secondary)]">{MAX_PLAN_MARKETING.tagline}</p>
              ...
              {status.authenticated ? (
                <button
                  ...
                  onClick={() => void primaryAction()}
                  disabled={isPrimaryDisabled}
                  className="mt-8 inline-flex w-full items-center justify-center rounded-xl bg-[var(--primary)] px-5 py-3 text-sm font-semibold text-white transition-colors hover:bg-[var(--primary-hover)] disabled:cursor-not-allowed disabled:opacity-60"
                >
                  {action === 'checkout'
                    ? 'Abrindo checkout...'
                    : action === 'portal'
                ...
                <Link
                  href="/register"
                  className="mt-8 inline-flex w-full items-center justify-center rounded-xl bg-[var(--primary)] px-5 py-3 text-sm font-semibold text-white transition-colors hover:bg-[var(--primary-hover)]"
                >
                  Criar conta e testar Max
                </Link>
              )}
      ...
      <section className="py-16 md:py-24">
        <div className="container">
          <div className="mb-10 flex flex-col justify-between gap-4 md:flex-row md:items-end">
            ...
            <span className="inline-flex w-fit items-center gap-2 rounded-full border border-[var(--border-color)] px-3 py-1.5 text-xs font-semibold text-[var(--text-secondary)]">
              <ShieldIcon />
              Stripe seguro
            </span>
          </div>
          ...
          <div className="overflow-hidden rounded-2xl border border-[var(--border-color)] bg-[var(--card-bg)]">
            <div className="grid grid-cols-[1.2fr_0.9fr_0.9fr] border-b border-[var(--border-color)] bg-[var(--bg-surface)] px-4 py-3 text-xs font-semibold uppercase tracking-wider text-[var(--text-muted)] sm:px-6">
              <span>Recurso</span>
            ...
            {PLAN_COMPARISON_ROWS.map((row) => (
              <div
                key={row.feature}
                className="grid grid-cols-[1.2fr_0.9fr_0.9fr] gap-3 border-b border-[var(--border-color)] px-4 py-4 text-sm last:border-b-0 sm:px-6"
              >
                <span className="font-medium text-[var(--text-primary)]">{row.feature}</span>
                <span className="text-[var(--text-secondary)]">{row.free}</span>
                <span className="font-semibold text-[var(--primary)]">{row.max}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="border-t border-[var(--border-color)] bg-[var(--bg-surface)] py-16 md:py-20">
        <div className="container">
            ...
            <div className="mt-8 flex flex-wrap justify-center gap-3">
              {status.authenticated ? (
                <button
                  ...
                  onClick={() => void primaryAction()}
                  disabled={isPrimaryDisabled}
                  className="inline-flex items-center justify-center rounded-xl bg-[var(--primary)] px-6 py-3 text-sm font-semibold text-white transition-colors hover:bg-[var(--primary-hover)] disabled:cursor-not-allowed disabled:opacity-60"
                >
                  {action === 'checkout'
                    ? 'Abrindo checkout...'
                    : action === 'portal'
                ...
                <Link
                  href="/register"
                  className="inline-flex items-center justify-center rounded-xl bg-[var(--primary)] px-6 py-3 text-sm font-semibold text-white transition-colors hover:bg-[var(--primary-hover)]"
                >
                  Criar conta e testar Max
                </Link>
              )}
              {status.authenticated && hasStripeCustomer && !subscription.hasMaxAccess && (
                <button
                  ...
                  className="inline-flex items-center justify-center rounded-xl border border-[var(--border-color)] px-6 py-3 text-sm font-semibold text-[var(--text-secondary)] transition-colors hover:bg-[var(--bg-elevated)] hover:text-[var(--text-primary)] disabled:cursor-not-allowed disabled:opacity-60"
                >
