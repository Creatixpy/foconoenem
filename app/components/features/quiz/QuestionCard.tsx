...
export default function QuestionCard({
  ...
  const disciplineColor = DISCIPLINE_COLORS[question.discipline] || 'var(--primary)';
  ...
  function getAltStyle(alt: (typeof question.alternatives)[0]) {
    if (answerState === 'revealed') {
      if (alt.isCorrect) {
        return 'border-[var(--accent)] bg-[var(--accent)]/10 ring-1 ring-[var(--accent)]/30';
      }
    ...
    if (alt.id === selectedAnswer) {
      return 'border-[var(--primary)] bg-[var(--primary)]/10 ring-1 ring-[var(--primary)]/30';
    }
    return 'border-[var(--border-color)] hover:border-[var(--border-hover)] hover:bg-[var(--bg-surface)]';
  }
  ...
  function getAltIcon(alt: (typeof question.alternatives)[0]) {
    ...
    if (alt.isCorrect) {
      return (
        <span className="flex-shrink-0 w-5 h-5 rounded-full bg-[var(--accent)] flex items-center justify-center">
  ...
  return (
    ...
    >
      ...
      {/* question text */}
      <div className="rounded-xl border border-[var(--border-color)] bg-[var(--card-bg)] p-5 sm:p-6 mb-5">
        <p className="text-[var(--text-primary)] text-base sm:text-lg leading-relaxed whitespace-pre-line">
          {question.text}
        </p>
      </div>
        ...
        {question.alternatives.map((alt, i) => {
          ...
          return (
            ...
            >
              ...
              <span className="text-sm text-[var(--text-primary)] leading-relaxed flex-1 pt-0.5">
                {alt.text}
              </span>
              {getAltIcon(alt)}
            </button>
      ...
      {answerState === 'revealed' && (
        ...
        >
          ...
          {question.explanation && (
            <div className="rounded-xl border border-[var(--border-color)] bg-[var(--card-bg)] overflow-hidden">
              <button
                ...
                className="w-full flex items-center justify-between p-4 cursor-pointer hover:bg-[var(--bg-surface)] transition-colors"
              >
          ...
          <button
            onClick={onNext}
            className="w-full sm:w-auto px-6 py-3 rounded-lg bg-[var(--primary)] text-white font-medium hover:bg-[var(--primary-hover)] transition-colors cursor-pointer flex items-center gap-2"
          >
