                    ...
                    {question.alternatives.map((alternative) => {
                        ...
                        return (
                            <button
                                ...
                                type="button"
                                className={`relative flex w-full flex-col gap-3 rounded-2xl border-0 p-4 text-left transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-primary/60 focus-visible:ring-offset-2 focus-visible:ring-offset-background sm:flex-row sm:items-center sm:gap-4 ${isSelected && !showResults ? "bg-primary/5 shadow-md" : "bg-muted-bg/30 hover:bg-muted-bg/50"
                                    } ${showAsCorrect ? "bg-success/10" : ""} ${showAsWrong ? "bg-danger/10" : ""}`}
                                aria-pressed={isSelected}
                                aria-label={`Alternativa ${alternative.id}`}
                            >
                                <div className="flex w-full items-start gap-4">
                                    <span className={`flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full border-0 text-sm font-semibold transition-colors sm:h-10 sm:w-10 ${showAsCorrect
