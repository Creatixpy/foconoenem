...
export default function Header() {
  ...
  const outlineTone = isScrolled ? "text-foreground/80 hover:text-primary" : "text-foreground/80 hover:text-primary";
        ...
        <div className="flex items-center gap-3 md:gap-4">
          {user ? (
            ...
            <>
              <Link
                href="/auth/login"
                className={`hidden rounded-full px-4 py-2 text-sm font-semibold transition-colors md:inline-block ${outlineTone}`}
              >
                Entrar
              </Link>
              <Link
                href="/auth/register"
                className="rounded-full px-4 py-2 text-sm font-semibold text-white transition-colors shadow-sm bg-primary hover:bg-primary-dark"
              >
                Criar conta
              </Link>
            </>
